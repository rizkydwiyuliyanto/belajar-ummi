...
import { Box, Button, Dialog, DialogActions, DialogContent, DialogTitle, Grid, IconButton, Paper, Stack } from '@mui/material';
            ...
            <CustomContainer>
                <ContentParent>
                    <CustomPaper Title={"Isi buku"}>
                        {!loading &&
                            <>
                                <Toggles Value={value} SetValue={setValue} />
                                {value === "grid" &&
                                    <>
                                ...
                                }
                                {
                                    value === "slide" &&
                                    <>
                                        <SlideShow Length={result.length}>
                                            {result.map((x: any, index: number) => {
                                                const src = `${showImage({ filePath: x?.file_foto })}`;
                                                return (
                                                    <Paper>
                                                        <Grid item xs={12} sm={4} key={index}>
                                                            <Box
                                                                sx={{ width: "100%", margin: "0 auto", height: "600px" }}
                                                            >
                                                                <img
                                                                    src={src}
                                                                    alt={`Page ${index + 1}`}
